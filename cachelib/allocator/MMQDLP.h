 ...
 public:
  ...
  struct Config {
    ...
    Config(bool updateOnW, bool updateOnR)
        : Config(updateOnW, updateOnR, false, 0) {}
    ...
    Config(bool updateOnW, bool updateOnR, uint8_t ipSpec)
        : Config(updateOnW, updateOnR, false, ipSpec) {}

    Config(bool updateOnW,
           bool updateOnR,
           bool tryLockU,
           uint8_t ipSpec)
        : Config(updateOnW, updateOnR, tryLockU, ipSpec, 0, false) {}
  ...
  struct Container {
   private:
    using FIFOList = DList<T, HookPtr>;
